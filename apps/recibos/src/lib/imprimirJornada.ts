...
import { armarJornada, castigoTotal, type JornadaImpresa } from "./jornadaTexto";
import type { Bitacora, Compania, Recibidor, TipoCafe, Transportista } from "../db/models";

/**
 * Reúne los datos de la jornada y la manda a la impresora.
 *
 ...
 */
export async function imprimirJornada(bitacora: Bitacora): Promise<void> {
  await imprimirTexto(armarJornada(await reunirJornada(bitacora)));
}

async function reunirJornada(bitacora: Bitacora): Promise<JornadaImpresa> {
  ...
  return {
    ...
    // marcar `horaFinal`, justamente para que un fallo de la impresora deje la jornada
